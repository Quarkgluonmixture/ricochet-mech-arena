...
import { type Aabb, type Vec2, add, expandAll, forward, pointSegmentDist, scale } from './geom.ts';
...
export class World {
  ...
  private fire(m: Mech): void {
    ...
    const muzzle = add(m.pos, scale(f, m.radius + CFG.shell.radius + 0.25));
    const s: Shell = {
  ...
  private hitTest(s: Shell): void {
    for (const m of this.mechs) {
      ...
      if (this.roundResetAt < 0) this.roundResetAt = this.time + CFG.round.respawnDelay;
      return;
    }
  }
