...
view.addEventListener('click', () => { if (!locked) play(); });
...
function simStep(input: MechInput | null): void {
  world.step(STEP, [input, aiThink(world, enemy, player, aiState, STEP)]);
  handleEvents();
}
...
function frame(now: number): void {
  ...
  if (locked) {
    acc += dt;
    while (acc >= STEP) { simStep(playerInput()); acc -= STEP; }
  }
  playerView.update(player, dt);
  enemyView.update(enemy, dt);
  playerView.root.visible = player.alive && rig.mode === 'third';
...
window.rma = {
  world, player, enemy, aiState, CFG, rig, hud, stats,
  ...
  probe() {
    return {
      time: world.time, round: world.round, shells: world.shells.length, locked, mode: rig.mode,
